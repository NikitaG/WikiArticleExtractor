...
import WikiCoordinateExtractor
...
class Extractor(object):
    ...
    def extract(self, out, extractCoordinates, importSql):
        """
        :param out: a memory file.
        """
        ...
        if extractCoordinates:
            WikiCoordinateExtractor.process(self, text)
        ...
        if importSql:
            WikiCoordinateExtractor.output(self, text)
        else:
    ...
    while True:
        ...
        if job:
            ...
            try:
                ...
                page = None              # free memory
                e.extract(out, opts.extract_coordinates, opts.import_sql)
                text = out.getvalue()
            except:
                text = ''
...
def main():
    ...
    groupO.add_argument("--sql", action="store_true",
                        help="write output to sql")


    groupP = parser.add_argument_group('Processing')
    groupP.add_argument("--html", action="store_true",
                        help="produce HTML output, subsumes --links")
    groupP.add_argument("--coordinates", action="store_true",
                        help="extracts coordinates from articles")
    ...
    options.extract_coordinates = args.coordinates
    options.write_json = args.json
    options.import_sql = args.sql
